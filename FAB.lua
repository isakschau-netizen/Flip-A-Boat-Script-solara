...
TeleportTab:CreateDropdown({
    ...
    Callback = function(Option)
        ...
        if Option == "1. Location One" then
            targetPos = Vector3.new(3175, 9, 3323)
        elseif Option == "2. Location Two" then
            targetPos = Vector3.new(-853, 19, -6086)
        elseif Option == "3. Location Three" then
            targetPos = Vector3.new(5886, 20, -3009)
        elseif Option == "4. Location Four" then
            targetPos = Vector3.new(-3580, 12, 2457)
        end
...
TeleportTab:CreateButton({
    Name = "Teleport to Junkyard",
    Callback = function()
        if hrp then
            local targetPos = Vector3.new(-724, 5, -1666)
            local tweenInfo = TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local tween = TweenService:Create(hrp, tweenInfo, {CFrame = CFrame.new(targetPos)})
            tween:Play()
